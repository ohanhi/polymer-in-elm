...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var toProp = F2(function (propName,num) {    return A2($Html$Attributes.property,propName,$Json$Encode.$float(num));});
   var lat = function (num) {    return A2(toProp,"latitude",num);};
   var lng = function (num) {    return A2(toProp,"longitude",num);};
   var zoom = function (num) {    return A2(toProp,"zoom",num);};
   var googleMapMarker = $Html.node("google-map-marker");
   var googleMap = $Html.node("google-map");
   var coords = F2(function (_p0,tick) {    var _p1 = _p0;return {ctor: "_Tuple2",_0: _p1._0,_1: _p1._1 + tick};});
   var futu = {ctor: "_Tuple2",_0: 60.1679729,_1: 24.9350682};
   var view = function (_p2) {
      var _p3 = _p2;
      var _p5 = _p3._1;
      var _p4 = _p3._0;
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "padding",_1: "2em"}]))]),
      _U.list([A2($Html.h2,_U.list([]),_U.list([$Html.text("Custom Elements with Elm")]))
              ,A2($Html.p,_U.list([]),_U.list([$Html.text("This application is written in Elm, using the\n      <google-map> Polymer element.")]))
              ,A2($Html.p,_U.list([]),_U.list([$Html.text("The longitude is updated based on a Time.every signal within\n      the Elm app.")]))
              ,A2($Html.p,
              _U.list([]),
              _U.list([$Html.text(A2($Basics._op["++"],"Latitude: ",$Basics.toString(_p4)))
                      ,A2($Html.br,_U.list([]),_U.list([]))
                      ,$Html.text(A2($Basics._op["++"],"Longitude: ",$Basics.toString(_p5)))]))
              ,A2(googleMap,
              _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "height",_1: "400px"},{ctor: "_Tuple2",_0: "width",_1: "400px"}]))
                      ,zoom(5)
                      ,lat(_p4)
                      ,lng(_p5)]),
              _U.list([A2(googleMapMarker,_U.list([lat($Basics.fst(futu)),lng($Basics.snd(futu))]),_U.list([]))]))]));
   };
   var initTick = 0;
   var ticks = A3($Signal.foldp,F2(function (_p6,tick) {    return tick + 1;}),initTick,$Time.every($Time.second));
   var main = A2($Signal.map,view,A2($Signal.map,coords(futu),ticks));
